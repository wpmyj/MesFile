...
namespace LY.MES.WFCL.Pipeline
{
    public partial class PipelineOperate : BaseForm
    {
            ...
            :base(context)
        {
            ...
            Client.Utility.DataEnum.GetEnumData(context, "PipelineStatus").ForEach(item => PipelineStaus.Add(item.Item2));
        ...
        private void GetPipelineOpereate(string PipelineCode)
        {
            try
            {
                using (var client = new SRPiPeline.PipelineServiceClient())
                {
                    var ret = client.GetDaviceData(context.SessionID, PipelineCode);
                    if (ret.IsSuccess)
                    {
                        Model = new SRPiPeline.MainPipelineInfo();
                        Model.Operator = context.UserName;
